...
func Test_padNumbers(t *testing.T) {
	...
	tests := []nameMapTest{
		...
		{ // 5G is not split here because it has another capital 'E' just after it
			...
			expectpath:      "connectivity-services-connectivity-service-core5g-endpoint",
			skipped:         2,
		},
		{
			openapiName:     "SiteSliceConnectivityService",
			expectSplitPath: []string{"Site", "Slice", "Connectivity", "Service"},
			expectpath:      "site-slice-connectivity-service",
			skipped:         0,
		},
		{
			openapiName:     "Cont1AList5",
			expectSplitPath: []string{"Cont", "1", "A", "List", "5"},
			expectpath:      "cont1a-list5",
			skipped:         3,
		},
		{
			openapiName:     "Cont101AList501B",
			expectSplitPath: []string{"Cont", "1", "0", "1", "A", "List", "5", "0", "1", "B"},
			expectpath:      "cont101a-list501b",
			skipped:         8,
		},
		{
			openapiName:     "Cont1A",
	...
	for _, test := range tests {
		...
		pathPartsJoined := strings.ToLower(strings.Join(test.expectSplitPath, "-"))
		detaggedNumbers, times := deTagNumbers(pathPartsJoined)
		assert.Equal(t, test.skipped, times, test.openapiName)
		assert.Equal(t, test.expectpath, detaggedNumbers, test.openapiName)
	}

	//const cont15a = "Cont15A"
	//assert.Equal(t, "cont-1-a", splitPath(cont15a))
	//assert.Equal(t, "cont-15a", deTagNumbers("cont15a"), "2 consecutive digits test")

	//assert.Equal(t, "cont-1a", deTagNumbers("cont1a"), "single digit test")
	//assert.Equal(t, "conta", deTagNumbers("conta"), "no digits test")
	//assert.Equal(t, "conta-1", deTagNumbers("conta1"), "last char is digit test")
	//assert.Equal(t, "cont-21a-22b", deTagNumbers("cont21a22b"), "multiple sets of digits test")
	//assert.Equal(t, "cont-21a-21b", deTagNumbers("cont21a21b"), "same sets of digits test")
	//assert.Equal(t, "cont-21a-21b-212c-221d", deTagNumbers("cont21a21b212c221d"), "same digits grouped differently")
	//const Core5gEndpoint = "Core5gEndpoint"
	//assert.Equal(t, "core-5g-endpoint", splitPath(Core5gEndpoint))
	//assert.Equal(t, splitPath(Core5gEndpoint), deTagNumbers("core-5g-endpoint"), "lower case after number")
}
