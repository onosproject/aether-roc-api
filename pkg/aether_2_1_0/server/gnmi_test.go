...
func Test_gnmiGetAetherV200targetSubscriber(t *testing.T) {
	...
	apResource, err := serverImpl.gnmiGetAccessProfile(
		context.Background(), "/aether/v2.1.0/internal/access-profile", "internal")
	...
	apnResource, err := serverImpl.gnmiGetApnProfile(
		context.Background(), "/aether/v2.1.0/internal/apn-profile", "internal")
	assert.NilError(t, err, "unexpected error on GetRequest")
	...
	subscriberResource, err := serverImpl.gnmiGetSubscriber(
		context.Background(), "/aether/v2.1.0/internal/subscriber", "internal")
	assert.NilError(t, err, "unexpected error on GetRequest")
